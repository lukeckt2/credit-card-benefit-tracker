...
function categoryLabel(value) {
  return categoryLabels[value] || (value ? String(value).replace(/_/g, " ").replace(/\b\w/g, (l) => l.toUpperCase()) : "Uncategorised");
}

function categoryGroupHeader(category) {
  const tr = document.createElement("tr");
  tr.className = "category-group-header";
  tr.innerHTML = `<td colspan="8"><span class="category-group-label">${escapeHtml(categoryLabel(category))}</span></td>`;
  return tr;
}
...
function renderTable(section, body, countElement, emptyMessage) {
  ...
  const groups = Array.isArray(section.category_groups) ? section.category_groups : null;

  if (!rows.length) {
    fragment.appendChild(emptyRow(emptyMessage));
  } else if (groups) {
    groups.forEach((group) => {
      fragment.appendChild(categoryGroupHeader(group.category));
      (Array.isArray(group.rows) ? group.rows : []).forEach((row) =>
        fragment.appendChild(benefitRow(row)),
      );
    });
  } else {
